...
def getleechinfo(from_user):
    ...
    if (
        ...
        ltype = "DOCUMENT"
        buttons.sbutton("🖥️ Send As Media 🖥️", f"leechset {user_id} med")
    else:
        ltype = "MEDIA"
    ...
    if os.path.exists(thumbpath):
        thumbmsg = "Exists"
        buttons.sbutton("🗑️ Delete Thumbnail ❓", f"leechset {user_id} thumb")
    else:
        thumbmsg = "Not Exists"

    if AUTO_DELETE_MESSAGE_DURATION == -1:
        buttons.sbutton("Close", f"<b>Leechset {user_id}</b>🤗 close")
